...
class UsbChannel(
    ...
    override fun read(buffer: ByteArray) {
        ...
        while (totalRead < buffer.size) {
            val res = connection.bulkTransfer(epIn, buffer, totalRead, buffer.size - totalRead, 3000)
    ...
    override fun write(buffer: ByteArray) {
        ...
        while (totalWritten < buffer.size) {
            val res = connection.bulkTransfer(epOut, buffer, totalWritten, buffer.size - totalWritten, 3000)
